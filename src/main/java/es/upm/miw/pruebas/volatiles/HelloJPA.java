...
public class HelloJPA {
    public static void main(String[] args) {
        ...
        Tema u1 = new Tema(1,"ÀEsta es una pregunta?", "Tema de prueba");
        // Create
        em.getTransaction().begin();
        em.persist(u1);
        em.persist(new Tema(2,"ÀPreguntas de deportes?", "Deportes"));
        em.persist(new Tema(3, "ÀPregunta de musica?", "Musica"));
        em.getTransaction().commit();
        // Read
        System.out.println(em.find(Tema.class, 1));
        // Update
        em.getTransaction().begin();
        em.merge(new Tema(2,"ÀPreguntas de tecnologia?", "dos cambiado con update"));
        em.getTransaction().commit();
        // Update2
        em.getTransaction().begin();
